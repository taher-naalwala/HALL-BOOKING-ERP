                        ...
                        <div class="form-group">
                            <div id="timing_date">
                                ...
                                <select placeholder="CHOOSE LABEL" name="label_id[]" class="choices-multiple-remove-button" multiple required>
                                    ...
                                    <?php $sql = "SELECT DISTINCT id,label,start_time,end_time from timings";
                                    $run = $conn->query($sql);
                                    if ($run->num_rows > 0) {
                                        while ($row = $run->fetch_assoc()) {
                                            $label = $row['label'];
                                    ...
                                    ?>
                                            <option value='<?php echo $timings_id  ?>'><?php echo $name . " (" . $final_start_time . " - " . $final_end_time . " )" ?></option>
                            ...
                            if (in_array("0", $jk_id)) {
                                ...
                                if ($run->num_rows > 0) {
                                    while ($row = $run->fetch_assoc()) {
                                        ...
                                        foreach ($label_id as $label_id_) {
                                            ...
                                            $s2 = "INSERT INTO blocked (`jk_id`, `date`, `label`, `label_id`, `type`) VALUES($id,'$date','$user_label',$label_id_,0)";
                                ...
                                foreach ($jk_id as $id) {
                                    foreach ($label_id as $label_id_) {
                                        ...
                                        $s2 = "INSERT INTO blocked (`jk_id`, `date`, `label`, `label_id`, `type`) VALUES($id,'$date','$user_label',$label_id_,1)";
                                        mysqli_query($conn, $s2);
                                    }
                                }
                            }
