            ...
            <div class="card-body" style="background-color: #F7F5E6;">
                ...
                <table class="table borderless table-responsive" width="100%" cellspacing="0">
                    ...
                    for ($i = $date_from; $i <= $date_to; $i += 86400) {
                        ...
                        $s2 = "SELECT amount from rent WHERE jk_id=$jk_id AND date>='$latest_date' ORDER BY date ASC LIMIT 1";
                        $run2 = $conn->query($s2);
                        if ($run2->num_rows > 0) {
                        ...
                        <tr>
                            <form method="POST" action="booking_admin.php">
                                ...
                                <?php while ($row1 = $run1->fetch_assoc()) {
                                    ...
                                    $s3 = "SELECT label from blocked WHERE (date='$latest_date' OR day='$day') AND timings_id=$id";
                                    $run3 = $conn->query($s3);
                                    if ($run3->num_rows > 0) {
                                        ...
                                        if ($run91->num_rows > 0) {
                                            ...
                                            while ($row91 = $run91->fetch_assoc()) {
                                                ...
                                                $s31_2 = "SELECT label from blocked WHERE (date='$latest_date' OR day='$day') AND timings_id=$c_id";
                ...
                <div class="card-body" style="background-color: #F7F5E6;">
                    <table class="table borderless" width="100%" cellspacing="0">
                        <tr>
                            ...
                            <?php
                            $sql = "SELECT  name,start_time,end_time from label order by id asc";
                            $run = $conn->query($sql);
                            if ($run->num_rows > 0) {
                                while ($row = $run->fetch_assoc()) {
                                    $label = $row['name'];
                        ...
                        if ($run->num_rows > 0) {
                            while ($row = $run->fetch_assoc()) {
                                ...
                                $s2 = "SELECT amount from rent WHERE jk_id=$jk_id AND date>='$f_first' ORDER BY date ASC LIMIT 1";
                                $run2 = $conn->query($s2);
                                if ($run2->num_rows > 0) {
                                    $row2 = $run2->fetch_assoc();
                                    $amount = $row2['amount'];
                        ...
                        ?>
                                <tr>
                                    <form method="POST" action="booking_admin.php">
                                        ...
                                        <td><?php echo $amount ?></td>
                                <?php
                                $sql_0 = "SELECT  id from label order by id asc";
                                $run_0 = $conn->query($sql_0);

                                while ($row_0 = $run_0->fetch_assoc()) {
                                    $label_id_ = $row_0['id'];
                                    $s1_0 = "SELECT id from timings WHERE jk_id=$jk_id AND label_id=$label_id_ ";
                                    $run1_0 = $conn->query($s1_0);
                                    if ($run1_0->num_rows > 0) {
                                        $row1 = $run1_0->fetch_assoc();
                                        $timings_id = $row1['id'];
                                        $s2 = "SELECT status from booking_info WHERE timings_id=$timings_id AND date='$f_first' AND (status=1 or status = 2 or status = 3) AND jk_id=$jk_id";
                                        $run2 = $conn->query($s2);
                                        if ($run2->num_rows > 0) {
                                            echo "<td><button class='btn btn-danger' disabled>Booked</button></td>";
                                        } else {
                                            ...
                                            if ($run91->num_rows > 0) {
                                                ...
                                                while ($row91 = $run91->fetch_assoc()) {
                                                    $c_jk_id = $row91['c_jk_id'];
                                                    $s101 = "SELECT id from timings WHERE jk_id=$c_jk_id AND label_id=$label_id_";
                                                    $run101 = $conn->query($s101);
                                                    $row101 = $run101->fetch_assoc();
                                                    $c_id = $row101['id'];
                                                    $s3 = "SELECT status from booking_info WHERE timings_id=$c_id AND date='$f_first' AND ($status=1 or $status == 2 or $status == 3) AND jk_id=$c_jk_id";
                                                    $run3 = $conn->query($s3);
                                                    if ($run3->num_rows > 0) {
                                                        $flag = 1;
                                                        break;
                ...
                ?>
                        <tr>
                            ...
                            while ($row_0 = $run_0->fetch_assoc()) {
                                ...
                                $s2 = "SELECT label_id from timings WHERE jk_id=$jk_id AND label_id=$label_id_ ORDER BY label_id ASC";
                                $run2 = $conn->query($s2);
                                if ($run2->num_rows > 0) {
                                    while ($row2 = $run2->fetch_assoc()) {
                                        $label_id = $row2['label_id'];
                                        $s0 = "SELECT start_time,end_time from label where id= $label_id";
                                        $run0 = $conn->query($s0);
                                        $row3 = $run0->fetch_assoc();
