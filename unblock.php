...
<head>
    ...
    <title>Unblock Jamaat Khana</title>
    <style>
        .btn-block {
        ...
        <div class="container-fluid">
            ...
            <div class="card shadow mb-4 ">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Unblock Jamaat Khana</h6>
                </div>
                <div class="card-body">
                    <form method="POST">
                    ...
                    if ($type == 1) {
                        ...
                        foreach ($dates as $date) {
                            ...
                            if (in_array("0", $jk_id)) {
                                    ...
                                    while ($row = $run->fetch_assoc()) {
                                        ...
                                        foreach ($label_id as $label_id_) {
                                            ...
                                            if ($run1->num_rows > 0) {
                                                ...
                                                //  $s2 = "INSERT INTO unblock (`jk_id`, `date`, `label`, `label_id`, `type`) VALUES($id,'$date','$user_label',$label_id_,0)";
                                                $s2 = "UPDATE blocked SET status=1,label='$user_label' WHERE timings_id=$timing_id and jk_id=$id and date='$date'";
                                ...
                                foreach ($jk_id as $id) {
                                    $s3 = "SELECT COUNT(*) as combine_count from combine_jk where o_jk_id=$id";
                                    $run3 = $conn->query($s3);
                                    $row3 = $run3->fetch_assoc();
                                    $combine_count = $row3['combine_count'];
                                    if ($combine_count > 0) {
                                        foreach ($label_id as $label_id_) {
                                        ...
                                        $s4 = "SELECT c_jk_id from combine_jk where o_jk_id=$id";
                                        $run4 = $conn->query($s4);
                                        while ($row4 = $run4->fetch_assoc()) {
                                            $c_jk_id = $row4['c_jk_id'];
                                            foreach ($label_id as $label_id_) {

                                                list($label, $start_time, $end_time) = explode(',', $label_id_);

                                                $s1 = "SELECT id from timings WHERE jk_id=$c_jk_id AND (label='$label' AND start_time='$start_time' AND end_time='$end_time')";
                                                $run1 = $conn->query($s1);
                                                if ($run1->num_rows > 0) {
                                                    $row1 = $run1->fetch_assoc();
                                                    $timing_id = $row1['id'];
                                                    ...
                                                    //  $s2 = "INSERT INTO unblock (`jk_id`, `date`, `label`, `label_id`, `type`) VALUES($id,'$date','$user_label',$label_id_,0)";
                                                    $s2 = "UPDATE blocked SET status=1,label='$user_label' WHERE timings_id=$timing_id and jk_id=$c_jk_id and date='$date'";
                                                    mysqli_query($conn, $s2);
                                                }
                                            }
                                        }
                                    ...
                                    if ($combine_count > 0) {
                                        foreach ($label_id as $label_id_) {
                                            ...
                                            if ($run1->num_rows > 0) {
                                                ...
                                                //  $s2 = "INSERT INTO unblock (`jk_id`, `date`, `label`, `label_id`, `type`) VALUES($id,'$date','$user_label',$label_id_,0)";
                                                $s2 = "UPDATE blocked SET status=1,label='$user_label' WHERE timings_id=$timing_id and jk_id=$id and date='$date' and type=1";
                                                mysqli_query($conn, $s2);
                                            }
                                        }
                                        $s4 = "SELECT c_jk_id from combine_jk where o_jk_id=$id";
